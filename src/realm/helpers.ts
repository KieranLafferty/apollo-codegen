...
import { Property, Struct } from '../swift/language';
...
export class Helpers {
  ...
  rootTypeNameFromGraphQLType(type: GraphQLType): string {
    if (type instanceof GraphQLNonNull) {
      return this.rootTypeNameFromGraphQLType(type.ofType);
    }

    let typeName;
    if (type instanceof GraphQLList) {
      typeName = this.rootTypeNameFromGraphQLType(type.ofType);
